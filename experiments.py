...
def run_experiment(
    func: EmbeddingCosine | PermutationShiftL1, opt_choice: str, seed: int, n_trials: int
) -> list[float]:
    ...
    for opt_name in opts:
        for func_name, func in funcs.items():
            for seed in range(n_seeds):
                values = run_experiment(func, opt_choice=opt_name, seed=seed, n_trials=n_trials)
