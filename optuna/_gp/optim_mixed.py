...
def _gradient_ascent(
    ...
    def negative_acqf_with_grad(scaled_x: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        (fval, grad) = acqf(normalized_params, with_grad=True)
...
def _exhaustive_search(
    ...
    fvals, _ = acqf(all_params, with_grad=False)
...
def _discrete_line_search(
    ...
    def negative_acqf_with_cache(i: int) -> float:
        ...
        # Flip sign because scipy minimizes functions.
        negval = -float(acqf(normalized_params, with_grad=False)[0])
...
def local_search_mixed(
    ...
    *,
    ...
    inverse_squared_lengthscales = acqf.inverse_squared_lengthscales
    ...
    best_fval = float(acqf(best_normalized_params, with_grad=False)[0])
...
def optimize_acqf_mixed(
    ...
    *,
    ...
    f_vals, _ = acqf(sampled_xs, with_grad=False)
