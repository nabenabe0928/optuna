...
def _compute_gp_posterior(
    ...
    mean_tensor, var_tensor = acqf.gpr.posterior(
        acqf.X,
        acqf.is_categorical,
        acqf.cov_Y_Y_inv,
        acqf.cov_Y_Y_inv_Y,
...
def _compute_gp_posterior_cov_two_thetas(
    ...
    acqf = acqf_module.LogEI(
        gpr=gpr, search_space=search_space, X=normalized_params, Y=standarized_score_vals
    )

    theta = torch.from_numpy(normalized_params[[theta1_index, theta2_index]])
    _, var = _posterior_of_batched_theta(
        acqf.gpr, acqf.X, acqf.is_categorical, acqf.cov_Y_Y_inv, acqf.cov_Y_Y_inv_Y, theta
    )
