...
def _calculate_weights_below_for_multi_objective(
    ...
    n_below_feasible = np.count_nonzero(feasible_mask)
    ...
    lvals = np.asarray([t.values for t in below_trials])[feasible_mask]
    lvals *= np.array([-1.0 if d == StudyDirection.MAXIMIZE else 1.0 for d in study.directions])
    ...
    weights_below[feasible_mask] = np.maximum(contribs / max(np.max(contribs), EPS), EPS)
    return weights_below
