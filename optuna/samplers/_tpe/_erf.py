...
qq = Polynomial([1.0, qq1, qq2, qq3, qq4, qq5])  # type: ignore[no-untyped-call]
...
qa = Polynomial([1.0, qa1, qa2, qa3, qa4, qa5, qa6])  # type: ignore[no-untyped-call]
...
sa = Polynomial([1.0, sa1, sa2, sa3, sa4, sa5, sa6, sa7, sa8])  # type: ignore[no-untyped-call]
...
sb = Polynomial([1.0, sb1, sb2, sb3, sb4, sb5, sb6, sb7])  # type: ignore[no-untyped-call]
...
def erf(x: np.ndarray) -> np.ndarray:
    ...
    def calc_case_tiny(a: np.ndarray) -> np.ndarray:
        # In the domain [0, 2**-28], only the first term in the power series expansion of erf(x) is
        # used.  The magnitude of the first neglected terms is less than 2**-84.
        return a + efx * a

    def calc_case_small1(a: np.ndarray) -> np.ndarray:
        z = a * a
        return (1 + pp(z) / qq(z)) * a

    def calc_case_small2(a: np.ndarray) -> np.ndarray:
        s = a - 1.0
        return erx + pa(s) / qa(s)

    def calc_case_med1(a: np.ndarray) -> np.ndarray:
        s = 1.0 / (a * a)
        R = ra(s)
        S = sa(s)
        ...
        # z = a
        # SET_LOW_WORD(z, 0)
        # r = np.exp(-z * z - 0.5625) * np.exp((z - a) * (z + a) + R / S)
        r = np.exp(-1.0 / s - 0.5625 + R / S)
        return 1.0 - r / a

    def calc_case_med2(a: np.ndarray) -> np.ndarray:
        s = 1.0 / (a * a)
        R = rb(s)
        S = sb(s)
        # z = a
    ...
    out = np.sign(x, dtype=float)
    for each_case, calc_func in zip(
        [case_tiny, case_small1, case_small2, case_med1, case_med2],
        [calc_case_tiny, calc_case_small1, calc_case_small2, calc_case_med1, calc_case_med2],
    ):
        b = a[each_case]
        if b.size:
            out[each_case] *= calc_func(b)
