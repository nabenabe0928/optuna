...
class GPSampler(BaseSampler):
    ...
    def _get_constraints_acqf_params(
        ...
        for i, (vals, mean, std) in enumerate(zip(C_train.T, means, stds)):
            cache = (
            ...
            gpr = gp.GPRegressor(
                is_categorical, X_train, vals, kernel_params=cache
        ...
        self._constraints_kernel_params_cache_list = [
            gpr.kernel_params for gpr in constraints_gprs
        ]
    ...
    def sample_relative(
        ...
        for i in range(n_objectives):
            ...
            gprs_list.append(
                gp.GPRegressor(
                    is_categorical, X_train, Y_train[:, i], kernel_params=cache
                ).fit_kernel_params(self._log_prior, self._minimum_noise, self._deterministic)
            )
        self._kernel_params_cache_list = [gpr.kernel_params for gpr in gprs_list]
        best_params: np.ndarray | None
