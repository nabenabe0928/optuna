...
def _compute_2d(solution_set: np.ndarray, reference_point: np.ndarray) -> float:
    ...
    This algorithm divides a hypervolume into smaller rectangles and sum these areas.
...
def _compute_3d(solution_set: np.ndarray, reference_point: np.ndarray) -> float:
    """Compute the hypervolume for the three-dimensional space.
...
def _try_compute_3d(solution_set: np.ndarray, reference_point: np.ndarray) -> float | None:
    global _sortedcontainers_exist
    if _sortedcontainers_exist:
        try:
            return _compute_3d(solution_set, reference_point)
        except ModuleNotFoundError:
            _sortedcontainers_exist = False
            return None
    else:
        # Lazy import takes mu seconds, which is a bottleneck, so we skip it from the second time.
        return None
