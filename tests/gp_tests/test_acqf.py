...
def test_eval_acqf(acqf_cls: type[BaseAcquisitionFunc], x: np.ndarray) -> None:
    n_dims = 2
    ...
    batched_acqf_value = acqf.eval_with_no_grad(x)
    assert batched_acqf_value.shape == x.shape[:-1]
    assert np.all(np.isfinite(batched_acqf_value))
