...
def test_wfg_2d(assume_pareto: bool) -> None:
    ...
    for n in range(2, 30):
        ...
        assert compute_hypervolume(s, r, assume_pareto) == n * n - n * (n - 1) // 2
...
def test_wfg_nd(n_objs: int, assume_pareto: bool) -> None:
    ...
    s = _shuffle_and_filter_sols(s, assume_pareto, rng)
    assert compute_hypervolume(s, r, assume_pareto) == 10**n_objs - 1
...
def test_wfg_with_inf(n_objs: int) -> None:
    ...
    s[0, n_objs // 2] = np.inf
    r = 1.1 * s
    assert compute_hypervolume(s, r) == np.inf
...
def test_wfg_duplicate_points(assume_pareto: bool) -> None:
    ...
    ground_truth = compute_hypervolume(s, r, assume_pareto=False)
    s = np.vstack([s, s[-1]])  # Add an already existing point.
    s = _shuffle_and_filter_sols(s, assume_pareto, rng)
    assert compute_hypervolume(s, r, assume_pareto) == ground_truth
